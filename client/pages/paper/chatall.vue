<template>
	...
	<view class="chat">
		...
		<scroll-view class="chat-scroll" scroll-y="true" :scroll-into-view="scrollInto" scroll-with-animation>
			<!-- 消息列表：绑定id，用于滚动到该元素  -->
			<chat-list v-for="(item,index) in list" :key="index" :item="item" :uid="userInformation.user_name"
...
<script>
	...
	export default {
		...
		mounted(){
			//var socket = io.connect("http:192.168.31.135:9090")
			/*this.ws = new WebSocket("ws://192.168.31.135:9090")
			this.ws.onmessage = (event)=>{
				var data = JSON.parse(event.data)
				//console.log(data)
				//console.log(this.userInformation.user_name);
				if(this.userInformation.user_name!=data.speaker)
				{
				this.list.push({
					userid: 1,
			...
			//const socket = io("http://192.168.31.135:9090",{transports:['websocket']})
			var that=this
			uni.onSocketMessage(function(res){
				...
				console.log(res.data)
				//console.log(this.userInformation.user_name);
				if(that.userInformation.user_name!=data.speaker)
				{
				that.list.push({
					userid: 1,
					avatar: '/static/img/header/a.svg',
		...
		methods: {
			...
			submit(data) {
				const obj = {
					userid: 1,
					avatar: '/static/img/header/a.svg',
					speaker: this.userInformation.user_name,
					data,
					type: 'text',
					createTime: this.$u.timeFormat(new Date(), 'yyyy-mm-dd hh:MM:ss')
				}
